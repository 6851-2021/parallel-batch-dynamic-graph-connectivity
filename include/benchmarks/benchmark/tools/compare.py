...
def main():
    ...
    json1 = json1_orig = include.benchmarks.benchmark.tools.gbench.util.run_or_load_benchmark(
        test_baseline, benchmark_options + options_baseline)
    json2 = json2_orig = include.benchmarks.benchmark.tools.gbench.util.run_or_load_benchmark(
        test_contender, benchmark_options + options_contender)

    # Now, filter the benchmarks so that the difference report can work
    if filter_baseline and filter_contender:
        replacement = '[%s vs. %s]' % (filter_baseline, filter_contender)
        json1 = include.benchmarks.benchmark.tools.gbench.report.filter_benchmark(
            json1_orig, filter_baseline, replacement)
        json2 = include.benchmarks.benchmark.tools.gbench.report.filter_benchmark(
            json2_orig, filter_contender, replacement)

    diff_report = include.benchmarks.benchmark.tools.gbench.report.get_difference_report(
        json1, json2, args.utest)
    output_lines = include.benchmarks.benchmark.tools.gbench.report.print_difference_report(
        diff_report,
